...
# Ensure required NLTK resources (for English)
import os as _os
# 将 NLTK 数据放在 text/en_userdic 下，类似 ja_userdic 的组织方式
_NLTK_LOCAL_DIR = _os.path.join(_os.path.dirname(__file__), "text", "en_userdic", "nltk_data")
_os.makedirs(_NLTK_LOCAL_DIR, exist_ok=True)
_os.environ.setdefault("NLTK_DATA", _NLTK_LOCAL_DIR)
...
def _ensure_nltk_resources():
    ...
    for path, name in resources:
        try:
            ...
            try:
                nltk.download(name, quiet=True, download_dir=_NLTK_LOCAL_DIR)
            except Exception:
                pass

_ensure_nltk_resources()

# Robust import for LangSegment with graceful fallback
try:
    import LangSegment as _LangSeg
    LangSeg = _LangSeg
except Exception:
    try:
        import langsegment as _LangSeg  # some dists use lowercase name
        LangSeg = _LangSeg
    except Exception:
        class _LangSegFallback:
            @staticmethod
            ...
            @staticmethod
            def getTexts(text):
                # Default: treat all as Chinese
                return [{"lang": "zh", "text": text}]

        LangSeg = _LangSegFallback()
...
def get_phones_and_bert(text,language):
    if language == "en":
        LangSeg.setfilters(["en"])
        text = " ".join(tmp["text"] for tmp in LangSeg.getTexts(text))
    elif language == "zh":
    ...
    textlist=[]
    langlist=[]
    for tmp in LangSeg.getTexts(text):
        if tmp["lang"] == "en":
...
def get_tts_wav(text, top_k=20, top_p=0.6, temperature=0.6, speed=1, refer=None, lang: str = "auto"):
    text = text.strip("/n")
    zero_wav = np.zeros(
    ...
    for i, text in enumerate(texts):
        if lang == "auto":
            try:
                text_language = _map_lang(detect(text))
            except Exception:
                text_language = "zh"
        else:
            text_language = _map_lang(lang)
...
def synthesize(GPT_model_path, SoVITS_model_path, target_text_path, output_path, lang: str = "auto"):
    # Read target text
    ...
    refers = [get_spepc(hps, "./tools/denoised/output/reference.wav").to(vq_precision).to(device)]

    # Synthesize audio
    synthesis_result = get_tts_wav(text=target_text, top_p=1, temperature=1, refer=refers, lang=lang)
...
def main():
    ...
    parser.add_argument('--lang', default='auto', help="Text language: auto|zh|en|ja|ko|yue (default: auto)")

    args = parser.parse_args()

    synthesize(args.gpt_model, args.sovits_model, args.target_text, args.output_path, args.lang)
