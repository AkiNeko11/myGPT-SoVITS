...
def plot_spectrogram_to_numpy(spectrogram):
    ...
    fig.canvas.draw()
    # Prefer modern API; fallback to legacy tostring_* for compatibility
    try:
        ...
        try:
            ...
            data = np.frombuffer(rgb_bytes, dtype=np.uint8)
            data = data.reshape(fig.canvas.get_width_height()[::-1] + (3,))
        except Exception:
            argb_bytes = fig.canvas.tostring_argb()
            argb = np.frombuffer(argb_bytes, dtype=np.uint8)
