...
os.environ["MASTER_ADDR"] = "localhost"  # or the IP address of your master node
...
def main():
    ...
    # On Windows or when only a single GPU is available, avoid DDP to bypass libuv requirement
    use_ddp_env = os.environ.get("S2_DDP", "0")  # set to "1" to force DDP
    if os.name == "nt" or n_gpus <= 1 or use_ddp_env != "1":
        run_single(hps)
    else:
        mp.spawn(
            run,
...
def train_and_evaluate(
    ...
    if epoch % hps.train.save_every_epoch == 0 and rank == 0:
        ckpt_dir = "%s/logs_s2" % hps.data.exp_dir
        # Always save step-numbered checkpoints for reliable恢复
        utils.save_checkpoint(
            net_g,
            optim_g,
            hps.train.learning_rate,
            epoch,
            os.path.join(ckpt_dir, "G_{}.pth".format(global_step)),
        )
        utils.save_checkpoint(
            net_d,
            optim_d,
            hps.train.learning_rate,
            epoch,
            os.path.join(ckpt_dir, "D_{}.pth".format(global_step)),
        )
        # Optionally also write human-readable latest aliases
        if hps.train.if_save_latest:
            utils.save_checkpoint(
                ...
                hps.train.learning_rate,
                epoch,
                os.path.join(ckpt_dir, "G_latest.pth"),
            )
            utils.save_checkpoint(
                net_d,
                optim_d,
                hps.train.learning_rate,
                epoch,
                os.path.join(ckpt_dir, "D_latest.pth"),
            )
        if rank == 0 and hps.train.if_save_every_weights == True:
            target_g = net_g.module if hasattr(net_g, "module") else net_g
            ckpt = target_g.state_dict()
            logger.info(
                "saving ckpt %s_e%s:%s"
                % (
                    hps.name,
...
def run_single(hps):
    """Single-process training path for Windows/single-GPU/CPU to avoid DDP/libuv."""
    global global_step
    rank = 0
    n_gpus = 1
    ...
    optim_g = torch.optim.AdamW(
        [
            {"params": filter(lambda p: p.requires_grad, net_g.parameters()), "lr": hps.train.learning_rate},
        ],
        hps.train.learning_rate,
        betas=hps.train.betas,
        eps=hps.train.eps,
