...
from text.symbols import punctuation, c as C_LIST, v_without_tone as V_LIST
...
# Try load mapping file; if missing, fallback to on-the-fly splitter
_mapping_path = os.path.join(current_file_path, "opencpop-strict.txt")
pinyin_to_symbol_map = {}
if os.path.exists(_mapping_path):
    pinyin_to_symbol_map = {
        line.split("\t")[0]: line.strip().split("\t")[1]
        for line in open(_mapping_path, encoding="utf-8").readlines()
    }
else:
    # Build mapping dynamically by splitting pinyin into initial/final
    C_LIST_SORTED = sorted(C_LIST, key=len, reverse=True)
    def _split_pinyin_to_cv(pinyin: str):
        initial = ""
        for cand in C_LIST_SORTED:
            if cand and pinyin.startswith(cand):
                rem = pinyin[len(cand):]
                if rem in V_LIST:
                    initial = cand
                    finals = rem
                    return initial, finals
        # no initial matched; entire pinyin should be finals, or may start with y/w already
        if pinyin in V_LIST:
            return "", pinyin
        # try y/w prefix split
        for cand in ["y", "w"]:
...
model_src = os.path.join(parent_directory,"pretrained_models", "chinese-roberta-wwm-ext-large")
try:
    g2pw = G2PWPinyin(model_dir=model_dir, model_source=model_src, v_to_u=False, neutral_tone_with_five=True)
    # Proceed with using g2pw if initialization is successful
except Exception as e:
    # Catch any exception and handle it
    print(f"Error initializing G2PWPinyin: {e}")
    # Optionally, you can handle the exception in another way, like setting g2pw to None
    g2pw = None
...
def _g2p(segments):
    ...
    for seg in segments:
        ...
        # g2pw采用整句推理；若不可用则回退到 pypinyin
        if g2pw is not None:
            pinyins = g2pw.lazy_pinyin(seg, neutral_tone_with_five=True, style=Style.TONE3)
        else:
            pinyins = lazy_pinyin(seg, neutral_tone_with_five=True, style=Style.TONE3)
        ...
        for c, v in zip(initials, finals):
            ...
            if c == v:
                ...
                if pinyin in pinyin_to_symbol_map:
                    ...
                    # Fallback split when mapping file is missing
                    ini, fin = _split_pinyin_to_cv(pinyin)
                    new_c, new_v = (ini if ini else fin, fin if ini else fin)
                new_v = new_v + tone
                phone = [new_c, new_v]
